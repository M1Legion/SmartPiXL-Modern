...
## CRITICAL POLICY: No Permission-Triggering APIs

**Effective:** February 7, 2026  
**Status:** IMPLEMENTED

The following browser APIs are **permanently excluded** from all fingerprinting tiers:

| Removed API | Reason |
|-------------|--------|
| `navigator.bluetooth` | Triggers permission prompt, zero fingerprint entropy |
| `navigator.usb` | Triggers permission prompt, zero fingerprint entropy |
| `navigator.serial` | Triggers permission prompt, zero fingerprint entropy |
| `navigator.hid` | Triggers permission prompt, zero fingerprint entropy |
| `navigator.requestMIDIAccess` | **Triggers alarming "control MIDI devices" popup** |
| `navigator.xr` | No permission prompt but zero fingerprint value |
| `navigator.geolocation` | Would require permission to get actual data |
| `Notification` | Would require permission to get actual data |
| `PushManager` | Would require permission to get actual data |
| `PaymentRequest` | Could trigger payment UI |
| `navigator.credentials` | Would require permission to get actual data |
| `navigator.share` | Could trigger share UI |
| `SpeechRecognition` | Could trigger mic permission |

**Rationale:** Our product must be completely seamless and invisible. Permission popups:
- Alarm end users who don't understand what these APIs are (especially MIDI in 2026)
- Make the product look like malware / black-hat tooling
...
| ID | Vulnerability | Severity | Effort | Impact | Status |
|----|---------------|----------|--------|--------|--------|
| V-01 | Canvas noise injection undetected | :red_circle: HIGH | Medium | High | :white_check_mark: IMPLEMENTED |
| V-02 | Audio noise injection undetected | :red_circle: HIGH | Medium | High | :white_check_mark: IMPLEMENTED |
| V-03 | No behavioral analysis | :red_circle: HIGH | High | Very High | :white_check_mark: IMPLEMENTED |
| V-04 | Stealth plugins bypass bot detection | :red_circle: HIGH | Medium | High | :white_check_mark: IMPLEMENTED |
| V-05 | Anti-detect browsers undetected | :orange_circle: MEDIUM | High | High | :white_check_mark: IMPLEMENTED |
| V-06 | Async data race condition | :orange_circle: MEDIUM | Low | Medium | :white_check_mark: IMPLEMENTED |
| V-07 | No TLS fingerprinting | :orange_circle: MEDIUM | High | High | :yellow_circle: HEADERS READY (needs proxy config) |
| V-08 | No datacenter IP detection | :orange_circle: MEDIUM | Medium | Medium | :white_check_mark: IMPLEMENTED |
| V-09 | Font detection easily spoofed | :yellow_circle: LOW | Low | Low | :white_check_mark: IMPLEMENTED |
| V-10 | Tor letterboxing incomplete | :yellow_circle: LOW | Low | Low | :white_check_mark: IMPLEMENTED |
