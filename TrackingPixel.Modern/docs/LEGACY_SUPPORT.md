...
Legacy <img> or <script>        Modern <script src>
        ...
        └───────────┬───────────────────┘
            ...
            (_srv_hitType = legacy | modern)
                    │
                PiXL.Test (same table, same 9 columns)
                    │
            usp_ParseNewHits
                    │
            PiXL.Parsed (HitType = 'legacy' → ~170 NULLs)
            PiXL.Visit  (HitType = 'legacy')
                    │
            usp_MatchVisits (modern: email-based)
                    │
            Future: usp_MatchLegacyVisits (IP+UA+TS)
```
...
## 8. Company/Pixel Sync from Xavier
