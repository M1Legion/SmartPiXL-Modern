...
// ============================================================================
// INFRASTRUCTURE HEALTH SERVICE — Sentinel edition.
//
...
public sealed class InfraHealthService : IDisposable
{
    ...
    private async Task<InfraHealthSnapshot> ProbeAllAsync()
    {
        ...
        // it's a diagnostic service cached for 15s, so Task.Run is appropriate.
    ...
    private List<ServiceHealthItem> ProbeWindowsServices()
    {
        var targets = new[]
        {
            ("MSSQL$SQL2025",       "SQL Server (SQL2025)",    true),
            ("W3SVC",               "IIS (World Wide Web)",    true),
            ("SmartPiXL-Forge",     "SmartPiXL Forge",         true),
            ("SQLAgent$SQL2025",    "SQL Agent (SQL2025)",     false),
        };
