...
// The Forge's FailoverCatchupService processes JSONL files on restart.

// JsonlFailoverService: Channel<T>-backed JSONL writer for durable failover when
// the named pipe to the Forge is unavailable. Daily rolling files in Failover/.
// Must be registered before PipeClientService (injected as a dependency).
builder.Services.AddSingleton<JsonlFailoverService>();
builder.Services.AddHostedService(sp => sp.GetRequiredService<JsonlFailoverService>());

// PipeClientService: Named pipe client that sends enriched TrackingData to the Forge.
// Channel<T> internally — TryEnqueue is lock-free for the hot path. Background loop
