-- ============================================================================
-- Migration 41: Add ScreenExtended + MousePath columns (Phase 1)
-- ============================================================================
-- Adds two new browser-collected fields to PiXL.Parsed:
--   ScreenExtended BIT           — window.screen.isExtended (multi-monitor)
--   MousePath      VARCHAR(2000) — raw mouse path x,y,t|x,y,t|... encoding
--
-- Updates ETL.usp_ParseNewHits:
--   Phase 1 (INSERT): ScreenExtended in Screen & Display group
--   Phase 7 (UPDATE): MousePath in Behavioral group
-- ============================================================================
PRINT '--- 41: Adding ScreenExtended + MousePath to PiXL.Parsed ---';
GO

-- =====================================================================
-- Step 1: Add columns to PiXL.Parsed
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'ScreenExtended'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD ScreenExtended BIT NULL;
    PRINT 'Added PiXL.Parsed.ScreenExtended (BIT NULL)';
END
ELSE
    PRINT 'PiXL.Parsed.ScreenExtended already exists — skipped';
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'MousePath'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD MousePath VARCHAR(2000) NULL;
    PRINT 'Added PiXL.Parsed.MousePath (VARCHAR(2000) NULL)';
END
ELSE
    PRINT 'PiXL.Parsed.MousePath already exists — skipped';
GO

-- =====================================================================
-- Step 2: Update ETL.usp_ParseNewHits — add ScreenExtended to Phase 1
--         INSERT and MousePath to Phase 7 UPDATE
-- =====================================================================
-- The full proc is re-created via CREATE OR ALTER to incorporate the
-- two new fields. This is the authoritative copy after migration 41.
-- =====================================================================
...
PRINT '--- 41: ScreenExtended + MousePath migration complete ---';
