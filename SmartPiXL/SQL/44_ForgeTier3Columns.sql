-- ============================================================================
-- Migration 44: Forge Tier 3 Enrichment Columns (Phase 6 — Asymmetric Detection)
-- ============================================================================
-- Adds 9 new columns to PiXL.Parsed for Tier 3 enrichment data produced
-- by SmartPiXL Forge asymmetric detection services:
--
--   Contradiction:   ContradictionCount, ContradictionList
--   Cultural:        CulturalConsistencyScore, CulturalFlags
--   Device age:      DeviceAgeYears, DeviceAgeAnomaly
--   Replay:          ReplayDetected, ReplayMatchFingerprint
--   Dead internet:   DeadInternetIndex
--
-- Updates ETL.usp_ParseNewHits:
--   New Phase 8D: Parses all _srv_* Tier 3 params via dbo.GetQueryParam()
-- ============================================================================
PRINT '--- 44: Adding Forge Tier 3 enrichment columns to PiXL.Parsed ---';
GO

-- =====================================================================
-- Step 1: Add columns — Contradiction Matrix
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'ContradictionCount'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD ContradictionCount INT NULL;
    PRINT 'Added PiXL.Parsed.ContradictionCount (INT NULL)';
END
ELSE PRINT 'PiXL.Parsed.ContradictionCount already exists — skipped';
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'ContradictionList'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD ContradictionList VARCHAR(500) NULL;
    PRINT 'Added PiXL.Parsed.ContradictionList (VARCHAR(500) NULL)';
END
ELSE PRINT 'PiXL.Parsed.ContradictionList already exists — skipped';
GO

-- =====================================================================
-- Step 2: Add columns — Cultural Consistency (Geographic Arbitrage)
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'CulturalConsistencyScore'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD CulturalConsistencyScore INT NULL;
    PRINT 'Added PiXL.Parsed.CulturalConsistencyScore (INT NULL)';
END
ELSE PRINT 'PiXL.Parsed.CulturalConsistencyScore already exists — skipped';
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'CulturalFlags'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD CulturalFlags VARCHAR(500) NULL;
    PRINT 'Added PiXL.Parsed.CulturalFlags (VARCHAR(500) NULL)';
END
ELSE PRINT 'PiXL.Parsed.CulturalFlags already exists — skipped';
GO

-- =====================================================================
-- Step 3: Add columns — Device Age Estimation
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'DeviceAgeYears'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD DeviceAgeYears INT NULL;
    PRINT 'Added PiXL.Parsed.DeviceAgeYears (INT NULL)';
END
ELSE PRINT 'PiXL.Parsed.DeviceAgeYears already exists — skipped';
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'DeviceAgeAnomaly'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD DeviceAgeAnomaly BIT NULL;
    PRINT 'Added PiXL.Parsed.DeviceAgeAnomaly (BIT NULL)';
END
ELSE PRINT 'PiXL.Parsed.DeviceAgeAnomaly already exists — skipped';
GO

-- =====================================================================
-- Step 4: Add columns — Behavioral Replay Detection
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'ReplayDetected'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD ReplayDetected BIT NULL;
    PRINT 'Added PiXL.Parsed.ReplayDetected (BIT NULL)';
END
ELSE PRINT 'PiXL.Parsed.ReplayDetected already exists — skipped';
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'ReplayMatchFingerprint'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD ReplayMatchFingerprint VARCHAR(200) NULL;
    PRINT 'Added PiXL.Parsed.ReplayMatchFingerprint (VARCHAR(200) NULL)';
END
ELSE PRINT 'PiXL.Parsed.ReplayMatchFingerprint already exists — skipped';
GO

-- =====================================================================
-- Step 5: Add columns — Dead Internet Index
-- =====================================================================
IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID(N'PiXL.Parsed')
      AND name = N'DeadInternetIndex'
)
BEGIN
    ALTER TABLE PiXL.Parsed ADD DeadInternetIndex INT NULL;
    PRINT 'Added PiXL.Parsed.DeadInternetIndex (INT NULL)';
END
ELSE PRINT 'PiXL.Parsed.DeadInternetIndex already exists — skipped';
GO

-- =====================================================================
-- Step 6: Update ETL.usp_ParseNewHits — Add Phase 8D for Tier 3
...
-- 9 new Tier 3 enrichment fields. This is the authoritative copy
-- after migration 44.
-- =====================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- =====================================================================
        -- PHASE 8C: UPDATE — Forge Tier 2 enrichment params (~10 UDF calls)
        -- =====================================================================
        UPDATE pp SET
            -- Cross-customer intelligence
            CrossCustomerHits  = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_crossCustHits') AS INT),
            CrossCustomerAlert = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_crossCustAlert') AS BIT),
            -- Lead quality scoring
            LeadQualityScore   = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_leadScore') AS INT),
            -- Session stitching
            SessionId          = dbo.GetQueryParam(src.QueryString, '_srv_sessionId'),
            SessionHitNumber   = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_sessionHitNum') AS INT),
            SessionDurationSec = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_sessionDurationSec') AS INT),
            SessionPageCount   = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_sessionPages') AS INT),
            -- Device affluence
            AffluenceSignal    = dbo.GetQueryParam(src.QueryString, '_srv_affluence'),
            GpuTier            = dbo.GetQueryParam(src.QueryString, '_srv_gpuTier')
        FROM PiXL.Parsed pp JOIN PiXL.Raw src ON pp.SourceId = src.Id
        WHERE pp.SourceId > @LastId AND pp.SourceId <= @MaxId;

        -- =====================================================================
        -- PHASE 8D: UPDATE — Forge Tier 3 enrichment params (~9 UDF calls)
        -- Asymmetric detection: Contradiction, Cultural, DeviceAge, Replay, DeadInternet
        -- =====================================================================
        UPDATE pp SET
            -- Contradiction matrix
            ContradictionCount       = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_contradictions') AS INT),
            ContradictionList        = dbo.GetQueryParam(src.QueryString, '_srv_contradictionList'),
            -- Cultural consistency (geographic arbitrage)
            CulturalConsistencyScore = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_culturalScore') AS INT),
            CulturalFlags            = dbo.GetQueryParam(src.QueryString, '_srv_culturalFlags'),
            -- Device age estimation
            DeviceAgeYears           = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_deviceAge') AS INT),
            DeviceAgeAnomaly         = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_deviceAgeAnomaly') AS BIT),
            -- Behavioral replay detection
            ReplayDetected           = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_replayDetected') AS BIT),
            ReplayMatchFingerprint   = dbo.GetQueryParam(src.QueryString, '_srv_replayMatchFP'),
            -- Dead internet index
            DeadInternetIndex        = TRY_CAST(dbo.GetQueryParam(src.QueryString, '_srv_deadInternetIdx') AS INT)
        FROM PiXL.Parsed pp JOIN PiXL.Raw src ON pp.SourceId = src.Id
        WHERE pp.SourceId > @LastId AND pp.SourceId <= @MaxId;
        ...
        USING (
            SELECT DISTINCT DeviceHash
            FROM #BatchRows
            WHERE DeviceHash IS NOT NULL
        ) AS source ON target.DeviceHash = source.DeviceHash
        WHEN NOT MATCHED THEN INSERT (DeviceHash) VALUES (source.DeviceHash);
        ...
        WHEN NOT MATCHED THEN INSERT (IPAddress, FirstSeen, LastSeen)
             VALUES (source.IPAddress, SYSUTCDATETIME(), SYSUTCDATETIME())
        WHEN MATCHED THEN UPDATE SET target.LastSeen = SYSUTCDATETIME();
...
PRINT '--- 44: Forge Tier 3 enrichment columns migration complete ---';
