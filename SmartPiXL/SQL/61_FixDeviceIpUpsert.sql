...
-- after migration 44.
-- =====================================================================
CREATE   PROCEDURE [ETL].[usp_ParseNewHits]
    @BatchSize INT = 10000
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -- =============================================================================
        -- PHASE 10: Upsert PiXL.Device (UPDATE + INSERT, no MERGE)
        -- =============================================================================
        CREATE TABLE #DeviceBatch (
            DeviceHash      VARBINARY(32) NOT NULL PRIMARY KEY,
            BatchFirstSeen  DATETIME2(3)  NOT NULL,
            BatchLastSeen   DATETIME2(3)  NOT NULL,
            BatchHitCount   INT           NOT NULL
        );

        INSERT INTO #DeviceBatch (DeviceHash, BatchFirstSeen, BatchLastSeen, BatchHitCount)
        SELECT DeviceHash, MIN(ReceivedAt), MAX(ReceivedAt), COUNT(*)
        FROM #BatchRows
        ...
        UPDATE d SET
            ...
            d.HitCount  = d.HitCount + b.BatchHitCount
        FROM PiXL.Device d
        INNER JOIN #DeviceBatch b ON d.DeviceHash = b.DeviceHash;
        ...
        FROM #DeviceBatch b
        WHERE NOT EXISTS (
            SELECT 1 FROM PiXL.Device d WHERE d.DeviceHash = b.DeviceHash
        );

        SET @DevicesUpserted = @DevicesUpserted + @@ROWCOUNT;
        DROP TABLE #DeviceBatch;
        ...
        -- =============================================================================
        -- PHASE 11: Upsert PiXL.IP (UPDATE + INSERT, no MERGE)
        -- =============================================================================
        CREATE TABLE #IpBatch (
        ...
        INSERT INTO #IpBatch (IPAddress, BatchFirstSeen, BatchLastSeen, BatchHitCount)
        SELECT IPAddress, MIN(ReceivedAt), MAX(ReceivedAt), COUNT(*)
        FROM #BatchRows
        ...
        UPDATE ip SET
            ...
            ip.LastSeen  = CASE WHEN b.BatchLastSeen > ip.LastSeen
                                THEN b.BatchLastSeen ELSE ip.LastSeen END,
            ip.HitCount  = ip.HitCount + b.BatchHitCount
        FROM PiXL.IP ip
        INNER JOIN #IpBatch b ON ip.IPAddress = b.IPAddress;
        ...
        FROM #IpBatch b
        WHERE NOT EXISTS (
            SELECT 1 FROM PiXL.IP ip WHERE ip.IPAddress = b.IPAddress
        );

        SET @IPsUpserted = @IPsUpserted + @@ROWCOUNT;
        DROP TABLE #IpBatch;
    ...
    BEGIN CATCH
        ...
        IF OBJECT_ID('tempdb..#DeviceBatch') IS NOT NULL DROP TABLE #DeviceBatch;
        IF OBJECT_ID('tempdb..#IpBatch') IS NOT NULL DROP TABLE #IpBatch;
        THROW;
    END CATCH;
END;
