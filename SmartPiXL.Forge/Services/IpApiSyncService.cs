...
//
// Ported from SmartPiXL.Worker-Deprecated/Services/IpApiSyncService.cs
...
/// to the local <c>SmartPiXL.IPAPI.IP</c> table on a configurable interval.
/// After syncing, clears the Edge's geo cache via <see cref="IEdgeHealthClient"/>.
/// </summary>
public sealed class IpApiSyncService : BackgroundService
{
        ...
        DateTime watermark, CancellationToken ct)
    {
        ...
        var (inserted, updated, maxLastSeen) = await WithDeadlockRetryAsync(async () =>
        {
            int ins = 0, upd = 0;
            DateTime? maxLs = null;
