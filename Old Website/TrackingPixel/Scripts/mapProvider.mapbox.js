(function($, global) {
    ...
    function loadExternals(mapboxJsOverride) {
        if (!mapboxJsOverride && isNotEdge() && isMapboxGlSupported()) {
            ...
            if (!loadedImpl) {
                return Promise.resolve($.DM.loadExternalScriptAsync('/_dm/s/crossPlatform/mapProvider.mapboxgl.js'))
                    .then(function () {
                        loadedImpl = $.geoProviders.mapbox.gl;
                    });
            }
        ...
        if (!loadedImpl) {
            return Promise.resolve($.DM.loadExternalScriptAsync('/_dm/s/crossPlatform/mapProvider.mapbox.leaflet.js'))
                .then(function () {
                    loadedImpl = $.geoProviders.mapbox.leaflet;
                });
        }
