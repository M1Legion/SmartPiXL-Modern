...
actionPos = InStr(ResponseString, """action""")
If actionPos > 0 Then
    Dim quoteStart, quoteEnd
    quoteStart = InStr(actionPos, ResponseString, """") ' Find first quote after the key
    quoteStart = InStr(quoteStart + 1, ResponseString, """") ' Opening quote of value
    quoteEnd   = InStr(quoteStart + 1, ResponseString, """") ' Closing quote of value
    If quoteStart > 0 And quoteEnd > 0 Then
        action = LCase(Mid(ResponseString, quoteStart + 1, quoteEnd - quoteStart - 1))
    End If
End If
