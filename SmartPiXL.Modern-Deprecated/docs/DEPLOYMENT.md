...
1. `PiXL.Raw` has rows but `PiXL.Parsed` doesn't → ETL hasn't run. Run `EXEC ETL.usp_ParseNewHits` manually.
2. Watermark ahead of max Raw ID → Reset:
   ```sql
...
### Brilliant PiXL (JavaScript — full fingerprinting)

```html
<script src="https://smartpixl.com/{CompanyID}/{PiXLID}_{domain}_SMART.js"></script>
```

Example:

```html
<script src="https://smartpixl.com/12800/00029_thetriviaquest.com_SMART.js"></script>
```

### Legacy PiXL (image tag — server-side only, no JS fingerprinting)

```html
<img src="https://smartpixl.com/{CompanyID}/{PiXLID}_{domain}_SMART.GIF"
     style="display: none !important;"
     http-equiv="Content-Style-Type" content="image/gif"/>
```

Example:

```html
<img src="https://smartpixl.com/12800/00029_thetriviaquest.com_SMART.GIF"
     style="display: none !important;"
     http-equiv="Content-Style-Type" content="image/gif"/>
```

> **URL schema:** `/{CompanyID}/{PiXLID}_{domain}_SMART.GIF` (legacy) or `_SMART.js` (brilliant).
> Domain is `smartpixl.com` in production. Do **not** use `smartpixl.info` — that is the internal hostname.
