/*
    20_ETLPhases9to13.sql
    =====================
    Extends ETL.usp_ParseNewHits with Phases 9-13:
      Phase  9: Compute DeviceHash from 5 fingerprint fields
      Phase 10: MERGE PiXL.Device (upsert by DeviceHash)
      Phase 11: MERGE PiXL.IP (upsert by IPAddress)
      Phase 12: Extract _cp_* client params → JSON (JSON_OBJECTAGG)
      Phase 13: INSERT PiXL.Visit (fact table, 1:1 with Parsed)

    PREREQUISITES:
      - SQL/19_DeviceIpVisitMatchTables.sql (PiXL.Device, IP, Visit must exist)
      - dbo.GetQueryParam UDF must exist
      - ETL.usp_ParseNewHits must already exist (Phases 1-8)

    TARGET: SQL Server 2025 (17.0.1050.2) — uses native json type, JSON_OBJECTAGG,
            CREATE JSON INDEX, STRING_SPLIT.

    NOTE: Uses CREATE OR ALTER to preserve existing permissions on the proc.
    The entire procedure body is replaced — Phases 1-8 are preserved verbatim,
    Phases 9-13 are appended before the watermark update.
...
BEGIN
    ...
    SELECT @MaxId = MAX(Id) FROM PiXL.Test;
    ...
    BEGIN TRY
        ...
        FROM PiXL.Test p WHERE p.Id > @LastId AND p.Id <= @MaxId;
        ...
        FROM PiXL.Parsed pp JOIN PiXL.Test src ON pp.SourceId = src.Id
        ...
        -- If ALL 5 are NULL (synthetic/bot hit), DeviceHash = NULL → no Device row created.
        ...
        -- IpType, IsDatacenter, DatacenterProvider are left NULL here —
        -- they'll be populated by a future enrichment pass or the C# IpClassificationService.
        ...
        -- =============================================================================
        -- Scans PiXL.Test.QueryString for params prefixed with '_cp_'.
        ...
        --
        -- If no _cp_* params exist for a row, ClientParamsJson stays NULL.
        -- Current data has 0 _cp_ params — this will activate when clients
        -- start embedding _cp_email, _cp_hid, etc. in the pixel URL.
        ...
        OUTER APPLY (
            SELECT JSON_OBJECTAGG(
                -- Key: strip '_cp_' prefix (4 chars) → e.g., '_cp_email' → 'email'
                SUBSTRING(s.value, 5, CHARINDEX('=', s.value + '=') - 5)
                :
                -- Value: use GetQueryParam for URL decoding (it handles %20, %2F, etc.)
            ...
            WHERE t.Id = b.SourceId
              AND s.value LIKE '[_]cp[_]%=_%'     -- literal _cp_ prefix + has = + has value
        ) cp
...
PRINT 'ETL.usp_ParseNewHits updated with Phases 9-13 (Device, IP, ClientParams, Visit)';
GO
